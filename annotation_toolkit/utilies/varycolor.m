...
elseif NumberOfPlots==3
    ColorSet=[0 1 0; 0 1 1; 1 0 0];
else %default and where this function has an actual advantage

    %we have 3 segments to distribute the plots
    EachSec=floor(NumberOfPlots/3); 
    
    %how many extra lines are there? 
    ExtraPlots=mod(NumberOfPlots,3); 
    ...
    for m=1:SecThree
        ColorSet(m+SecOne+SecTwo,:)=[1 0 (SecThree-m)/(SecThree)];
    end
