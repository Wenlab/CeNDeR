...
from common_utils.prints import fff, print_warning_message
from preprocessing.inference.anno2json import save_anno_as_json
from recognition.inference.feature_maker.utils import calc_cylindrical_rho_phi, calc_mean_pt_of_3d_neuron, calc_mean_pt_of_bbox
...
def locate_coordinate_pts_by_deeplearning(image):
    # TODO
    mass_of_center, anterior_y, posterior_y, ventral_x, dorsal_x = 0, 0, 0, 0, 0
    return mass_of_center, anterior_y, posterior_y, ventral_x, dorsal_x
...
def preprocess(params):
    ...
    group_struct = re.search(name_reg, stack_path)
    if group_struct:
        stack_name = group_struct.group()
    else:
        return
    ...
    try:
        ...
        print_warning_message(f"{stack_path}  {e}")
        return None

    results = dict()
    for idx, volume in enumerate(vols):
        volume_name = stack_name + "_" + fff(idx, 3)
        # main step
        result = preprocess_volume(volume = volume, mode = mode, load_path = os.path.join(load_path, stack_name, f"{fff(idx, 3)}.json") if load_path != "" else load_path)
        results[volume_name] = result

        if mode == 4:
            save_image(os.path.join(result_path, f"{fff(idx, 3)}.png"), result[1])
        elif mode == 5:
            image_path = f"{fff(idx, 3)}.png"
            save_image(os.path.join(result_path, image_path), result[1])
            save_anno_as_json(mode = mode,
                              json_path = os.path.join(result_path, f"{fff(idx, 3)}.json"),
