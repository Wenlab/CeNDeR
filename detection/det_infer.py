...
from detection.inference.network import MFDetectNetworkModule41
from detection.inference.nms import non_maximum_suppression_volume_func
    ...
    for vps in tqdm(np.array_split(list(vol_peaks.keys()), np.ceil(len(vol_peaks) / 30)), desc = "S.2 detection.ANN"):
        ...
        with torch.no_grad():
            for names, peaks, shifts, mrfs in dataloader:
                ...
                candidate_bboxes.extend(calc_predicted_bbox(p.tolist(), s.tolist(), d.tolist()) for p, s, d in zip(peaks[pos_idxes], shifts[pos_idxes], pred_delta[pos_idxes]))
...
# def filter_fp_multiprocess(volumes, result):
#     params = [[vol_ctn, volumes[vol_ctn]['image'], result[vol_ctn]] for vol_ctn in volumes.keys()]
#     with Pool(min(os.cpu_count() // 2, len(params))) as p:
#         result = {vol_ctn: res for vol_ctn, res in p.imap_unordered(filter_region_volume, params)}
#         return result
