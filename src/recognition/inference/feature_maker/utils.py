...
# ----- General --------------------
def count_coord_feature(coords: List[float], bins = 10, range = None, norm = False):
...
def calc_volume_mean(neurons: Dict):
    mx, my, mz = 0.0, 0.0, 0.0
    num = 0
    for neuron in neurons.values():
        for region in neuron:
            num += 1
            mx += (region[0] + region[2]) * 0.5
            my += (region[1] + region[3]) * 0.5
            mz += region[-1]
    mx, my, mz = mx / num, my / num, mz / num
    return mx, my, mz


def centralize_volume_neurons(neurons: Dict, means: List[float]) -> Dict:
    mx, my, mz = means
    neurons = {key: [[bb[0] - mx, bb[1] - my, bb[2] - mx, bb[3] - my, bb[4] - mz] for bb in neuron] for key, neuron in neurons.items()}  # [xmin, ymin, xmax, ymax, z]
    return neurons


def scale_volume_neurons(neurons: Dict, ratio: float) -> Dict:
    return {key: [[b * ratio for b in bb] for bb in neuron] for key, neuron in neurons}


def normalize_volume_neurons(neurons: Dict, means: List[float], ratio: float) -> Dict:
    mx, my, mz = means
    neurons = {key: [[(bb[0] - mx) * ratio, (bb[1] - my) * ratio, (bb[2] - mx) * ratio, (bb[3] - my) * ratio, (bb[4] - mz) * ratio] for bb in neuron] for key, neuron in
               neurons.items()}  # [xmin, ymin, xmax, ymax, z]
    return neurons
...
def scale_z_axis(neurons: Dict, z_scale: float) -> Dict:
    ...
    return {key: [[bb[0], bb[1], bb[2], bb[3], bb[4] * z_scale] for bb in value] for key, value in neurons.items()}
...
def normalize_neurons_range_4ptcloud(neurons, standard_diagonal_line: int or float = 1.0):
    """
    ...
    width, height = np.max(neurons[:, 0]) - np.min(neurons[:, 0]), np.max(neurons[:, 1]) - np.min(neurons[:, 1])
    scale = standard_diagonal_line / math.sqrt(width * width + height * height)
    modified_neurons = [[p[0] * scale, p[1] * scale, p[2] * scale] for p in neurons]  # for knn feature

    width_scale = height / width
    width_span = width * width_scale * scale
    height_span = height * scale
    z_span = (np.max(neurons[:, 2]) - np.min(neurons[:, 2])) * scale

    return modified_neurons, width_scale, [width_span, height_span, z_span]
