...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path",
                        default = "../data/benchmarks/fDNC/model.bin",
                        type = str)

    parser.add_argument("--eval_path", default = "../data/benchmarks/CeNDeR/ptcloud4fdnc/C1", type = str)  # the path to test data.
    ...
    parser.add_argument("--ref_idx", default = 0, type = int)  # 5 is used as template for neuropal data.
    ...
    parser.add_argument("--acc_name", default = 'CeNDeR', type = str)
    ...
    dev_data = neuron_data_pytorch(args.eval_path, batch_sz = args.batch_size, shuffle = False, mode = 'all',
                                   ref_idx = args.ref_idx, show_name = True, shuffle_pt = False, tmp_path = tmp_p, pt_mode = True)
    ...
    for batch_idx, data_batch in enumerate(dev_data_loader):
        ...
        for i in range(0, num_worm):
            ...
            idv_result.append(num_hit / num_match)
    ...
    print("------>>>>", np.sum(num_match_list), len(num_match_list), np.sum(num_label_list), len(num_label_list))
    print("---->>> tps: ", num_hit_all, "gts: ", num_match_all)
    print("---->>> average tps: ", num_hit_all / len(num_match_list))
